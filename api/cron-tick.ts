...
async function oauthSign(method: string, url: string): Promise<string> {
  ...
  const signatureBuffer = await crypto.subtle.sign("HMAC", cryptoKey, encoder.encode(base));
...
async function xPost(text: string): Promise<boolean> {
  ...
  const auth = await oauthSign("POST", "https://api.twitter.com/2/tweets");
  const res = await fetch("https://api.twitter.com/2/tweets", { method: "POST", headers: { Authorization: auth, "Content-Type": "application/json" }, body: JSON.stringify({ text }) });
  return res.ok;
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const lastTs = stateRow?.value ? parseInt(stateRow.value) : 0;
