...
async function xApi(method, url, body = "") {
  const res = await fetch(url, { method, headers: { Authorization: oauthSign(method, url, body), "Content-Type": "application/json" }, body: body || undefined });
  return res.json();
}
...
export default async function handler(req) {
  ...
  try {
    ...
    if (C.grok) {
      try {
        const r = await fetch("https://api.groq.com/openai/v1/chat/completions", { method: "POST", headers: {"Content-Type": "application/json", "Authorization": "Bearer " + C.grok}, body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{role: "user", content: prompt}], max_tokens: 60 }) });
