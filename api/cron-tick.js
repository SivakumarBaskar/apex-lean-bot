...
const C = { 
  ...
  minGrade: process.env.SCANNER_MIN_GRADE || "A+", cooldown: process.env.SCANNER_SIGNAL_COOLDOWN_HOURS || "4" 
};
...
const ASSETS = [
  ...
  { sym: "USD/CAD", pip: 0.0001, sessions: ["New York"], range: [1.30, 1.42], vol: 30 }, 
  { sym: "NZD/USD", pip: 0.0001, sessions: ["Sydney", "London"], range: [0.58, 0.68], vol: 32 }, 
  { sym: "USD/CHF", pip: 0.0001, sessions: ["London"], range: [0.82, 0.92], vol: 30 }, 
  { sym: "SPX500", pip: 0.1, sessions: ["New York"], range: [5100, 5600], vol: 150 }, 
  { sym: "NAS100", pip: 0.1, sessions: ["New York"], range: [18000, 20000], vol: 500 }, 
  { sym: "US30", pip: 1, sessions: ["New York"], range: [38000, 42000], vol: 300 }, 
...
async function xPost(text) { 
  ...
  try { 
    ...
    const res = await fetch("https://api.twitter.com/2/tweets", { 
      method: "POST", headers: { Authorization: oauthSign("POST", "https://api.twitter.com/2/tweets", b), "Content-Type": "application/json" }, 
      body: b 
    }); 
    return res.ok; 
...
async function getPrice(asset) { 
  if (asset.id) { 
    try { 
      ...
      if (d[asset.id] && d[asset.id].usd) return d[asset.id].usd; 
    } catch(e) {} 
  } 
  const m = Math.floor(Date.now() / 60000); 
  const s = m * 31 + asset.sym.split("").reduce((a, c) => a + c.charCodeAt(0), 0); 
...
async function scanOnce() { 
  ...
  const eligible = ASSETS.filter((a) => a.sessions.includes(session) || a.sessions.includes("24/7")); 
...
async function genAI(signal) {
  ...
  const htf = signal.dir === "BUY" ? "BULLISH" : "BEARISH"; 
  ...
  if (C.grok) { 
    try { 
      ...
      const d = await r.json(); 
      if (d.choices && d.choices[0]) return d.choices[0].message.content.replace(/"/g, "").slice(0, 280); 
    } catch(e) {} 
  } 
  
  const arrow = signal.dir === "BUY" ? "🟢" : "🔴"; 
  return arrow + " APEX SIGNAL — " + signal.sym + "\nHTF Bias: " + htf + " 😎\nEntry: " + e + " 📍\nSL: " + s + " | TP1: " + t1 + " 🚀\nR:R " + signal.rr + "\nTrigger: " + signal.confs.slice(0,2).join(" + ") + " 🛡️\nPure Price Action"; 
}
...
async function genFallbackAI(style) {
  ...
  else if (style === "thread") prompt = "Write the FIRST tweet of a 3-tweet thread teasing a Pure Price Action concept. End with 'Thread below 🧵'. Max 280 chars.";
  ...
  if (C.grok) { 
    try { 
      const r = await fetch("https://api.groq.com/openai/v1/chat/completions", { 
        method: "POST", 
        headers: {"Content-Type": "application/json", "Authorization": "Bearer " + C.grok}, 
        body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{role: "user", content: prompt}], max_tokens: 100 }) 
      }); 
      const d = await r.json(); 
      if (d.choices && d.choices[0]) return d.choices[0].message.content.replace(/"/g, "").slice(0, 280); 
    } catch(e) {} 
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await supabase.from("signals").insert({ 
        symbol: sig.sym, direction: sig.dir, entry: sig.entry, sl: sig.sl, tp1: sig.tp1, tp2: sig.tp2, 
        rr: sig.rr, confluences: sig.confs.join(","), grade: sig.grade, session: sig.session, style, content, posted: false 
      }).catch(() => {}); // Non-fatal DB error
      
      if (content) {
        const posted = await xPost(content);
        if (posted) {
          await supabase.from("signals").update({ posted: true }).eq("symbol", sig.sym).order("created_at", { ascending: false }).limit(1).single().catch(() => {});
          await supabase.from("logs").insert({ type: "SIGNAL", message: sig.dir + " " + sig.sym, details: content, success: true }).catch(() => {});
          await tgSend("✅ <b>Signal Posted</b>\n" + sig.dir + " " + sig.sym + " [" + sig.grade + "]\n" + content);
          return new Response(JSON.stringify({ status: "posted", type: "signal" }), { status: 200 });
        }
      }
    }

    // NO SIGNAL FOUND -> Post a Fallback Tweet
    if (!lastFallback || Date.now() - lastFallback >= fallbackCoolMs) {
      ...
      if (posted) {
        ...
        await supabase.from("logs").insert({ type: "FALLBACK", message: fallbackStyle, details: fallbackContent, success: true }).catch(() => {});
        await tgSend("📝 <b>Fallback Posted</b> (" + fallbackStyle + ")\n" + fallbackContent);
        return new Response(JSON.stringify({ status: "posted", type: fallbackStyle }), { status: 200 });
      }
    }

    return new Response(JSON.stringify({ status: "cooldown" }), { status: 200 });
  } catch (e) {
    // Completely silent catch - no Telegram spam, no crash, just return 500 to Vercel
    return new Response(JSON.stringify({ error: "bot_error" }), { status: 500 });
  }
}
