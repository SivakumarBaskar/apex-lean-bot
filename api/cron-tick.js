...
async function genAI(signal, style) {
  ...
  if (C.grok) {
    try {
      ...
      const d = await r.json();
      if (d.choices && d.choices[0] && d.choices[0].message) return d.choices[0].message.content.replace(/"/g, "").slice(0, 280);
    } catch(e) {}
  }
...
export default async function handler(req) {
  ...
  try {
    ...
    const lastTs = stateRow && stateRow.value ? parseInt(stateRow.value) : 0;
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const content = await genAI(sig, style);
    ...
    await supabase.from("logs").insert({ type: "ERROR", message: "Tick failed", details: msg, success: false }).catch(()=>{});
